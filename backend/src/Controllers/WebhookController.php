...
class WebhookController
{
    ...
    private function handleMessage(array $data): void
    {
        ...
        // Ignorar propios, grupos y status
        if ($fromMe) return;
        if (str_contains($remoteJid, '@g.us'))    return;
        if (str_contains($remoteJid, 'status@'))  return;

        // Extraer número real (soporte @s.whatsapp.net y @lid)
        $numero = '';
        ...
        $nombreDisplay = $pushName ?: ($contacto ? $contacto['nombre'] : $numero);
        ...
        if (!$lead) {
            ...
            // Auto-asignar según config de distribución
        ...
        $this->log("MSG OK lead=$leadId numero=$numero tipo=$tipo");

        // === AUTO-RESPUESTA ===
        $esNuevoLead = empty($lead); // era nuevo si no existía antes
    ...
    private function autoAssignLead(int $leadId, int $instanciaId): void
    {
        ...
        $maxChats = (int)($cfg['max_chats_por_usuario'] ?? 300);

        $stmt = $this->pdo->prepare("
            SELECT u.id, u.nombre,
                (SELECT COUNT(*) FROM crm_leads l WHERE l.usuario_asignado_id = u.id AND l.estado = 'asignado' AND l.instancia_id = ?) as chats_activos
            FROM usuarios u
            ...
            HAVING chats_activos < ?
            ORDER BY u.id ASC
        ");
        $stmt->execute([$instanciaId, $instanciaId, $maxChats]);
        $candidatos = $stmt->fetchAll(\PDO::FETCH_ASSOC);

        if (empty($candidatos)) {
            $this->log("AUTO-ASSIGN: sin candidatos online para instancia $instanciaId");
            return;
        }
        ...
        $this->log("AUTO-ASSIGN: lead $leadId -> usuario {$elegido['nombre']} (id={$elegido['id']}, modo=$modo)");
    }
}
