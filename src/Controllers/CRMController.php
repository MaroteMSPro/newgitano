...
class CRMController
{
    public function leads(): array
    {
        ...
        $tipo    = $_GET['tipo'] ?? 'chat';  // 'chat' (individuales) | 'grupo' | 'all'
        $page = max(1, (int)($_GET['page'] ?? 1));
        // Sin tope de 100: el frontend controla cuántos muestra (paginación local de a 50).
        $limit = max(10, min(9999, (int)($_GET['limit'] ?? 50)));
        $offset = ($page - 1) * $limit;

        $instanciaId = $_GET['instancia_id'] ?? '';

        // Backfill automático: cuando el usuario entra a "Grupos" o "Todos", materializar
        // como crm_leads todos los contactos tipo='grupo' que aún no tengan lead asociado.
        // Así los 100 grupos sincronizados aparecen en el CRM aunque no hayan recibido mensajes.
        // Es idempotente: si ya existen, el WHERE cl.id IS NULL los filtra.
        if ($tipo === 'grupo' || $tipo === 'all') {
            $backfillParams = [];
            $backfillSql = "
                INSERT INTO crm_leads
                    (instancia_id, contacto_id, numero, nombre, estado, usuario_asignado_id, created_at, updated_at)
                SELECT
                    c.instancia_id,
                    c.id,
                    c.numero,
                    COALESCE(c.nombre, c.numero),
                    'nuevo',
                    NULL,
                    NOW(),
                    NOW()
                ...
                LEFT JOIN crm_leads cl
                       ON cl.contacto_id = c.id
                      AND cl.instancia_id = c.instancia_id
                WHERE c.tipo   = 'grupo'
                  AND c.activo = 1
                  AND cl.id IS NULL
            ";
            if ($instanciaId !== '') {
                $backfillSql .= " AND c.instancia_id = ?";
                $backfillParams[] = $instanciaId;
            }
            try {
                $pdo->prepare($backfillSql)->execute($backfillParams);
            } catch (\Throwable $e) {
                // No romper el listado si el backfill falla — solo logear.
                error_log('[leads] backfill groups failed: ' . $e->getMessage());
            }
        }
        ...
        // Filtro por tipo de contacto (individual / grupo / todos)
        if ($tipo === 'chat') {
            $where[] = "(c.tipo = 'individual' OR c.tipo IS NULL OR l.contacto_id IS NULL)";
        } elseif ($tipo === 'grupo') {
            $where[] = "c.tipo = 'grupo'";
        }
        // si $tipo === 'all' → no se agrega filtro

        if ($search !== '') {
            // Busca en: nombre original, alias (nombre_personalizado), número, último mensaje, vendedor asignado
            $where[] = '(l.nombre LIKE ? OR l.nombre_personalizado LIKE ? OR l.numero LIKE ? OR l.ultimo_mensaje LIKE ? OR u.nombre LIKE ?)';
            $like = "%$search%";
            $params[] = $like;
            $params[] = $like;
            $params[] = $like;
        ...
        // Base de JOINs común para COUNT y SELECT (necesario porque search mira u.nombre y tipo mira c.tipo)
        $fromJoin = "FROM crm_leads l
            LEFT JOIN usuarios u  ON l.usuario_asignado_id = u.id
            LEFT JOIN contactos c ON l.contacto_id = c.id";

        $total = $pdo->prepare("SELECT COUNT(*) $fromJoin WHERE $whereStr");
        ...
        ");
        // Agregar userId al principio de params (corresponde al ? de la subquery de mensajes_sin_leer)
    ...
    public function attachFile(string $id): array
    {
        ...
        // ══════════════════════════════════════════════════════
        // PARCHE: Enviar archivos via Evolution API (audio, imagen, video, documento)
        // ══════════════════════════════════════════════════════
        $mime = $file['type'] ?? '';
        $isAudio = $_POST['is_audio'] ?? '0';
        $mediaType = $_POST['media_type'] ?? '';
        $fileName = $_POST['file_name'] ?? $file['name'];
        
        // Determinar tipo de media
        $esAudio = ($isAudio === '1') || (strpos($mime, 'audio/') === 0);
        $esImagen = ($mediaType === 'image') || (strpos($mime, 'image/') === 0);
        $esVideo = ($mediaType === 'video') || (strpos($mime, 'video/') === 0);
        $esDocumento = !$esAudio && !$esImagen && !$esVideo;
        
        if ($esAudio || $esImagen || $esVideo || $esDocumento) {
            // Obtener datos del lead (numero + instancia)
            $pdo = Database::connect();
            $lead = $pdo->prepare('SELECT numero, instancia_id FROM crm_leads WHERE id = ?');
            $lead->execute([$id]);
            $leadData = $lead->fetch();
            
            if ($leadData) {
                $numero = $leadData['numero'];
                $instanciaId = $leadData['instancia_id'];
                
                // Obtener nombre de instancia
                $inst = $pdo->prepare('SELECT nombre FROM instancias WHERE id = ?');
                $inst->execute([$instanciaId]);
                $instancia = $inst->fetchColumn();
                
                if ($instancia) {
                    $evo = new EvolutionService($instancia);
                    $fileB64 = base64_encode(file_get_contents($file['tmp_name']));
                    ...
                    try {
                        if ($esAudio) {
                            ...
                            $evo->enviarImagenBase64($numero, $fileB64, $caption, $mime);
                        } elseif ($esVideo) {
                            // Enviar video usando sendMedia con mediatype video
                            $this->enviarMediaEvolution($evo, $numero, $fileB64, 'video', $mime, $fileName, $caption);
                        } else {
                            // Enviar documento
                            $evo->enviarDocumento($numero, $fileB64, $fileName, $caption);
                        }
                    } catch (\Exception $e) {
                        // Log error pero continuar
                        error_log("Error enviando media: " . $e->getMessage());
                    }
                }
            }
        }
     ...
     * Crea un crm_lead a partir de un contacto existente.
     * Usado por el botón "Al CRM" en Contactos → Grupos.
     * Body: { "contacto_id": 123 }
     */
    public function fromContact(): array
    {
        $pdo  = Database::connect();
        $body = json_decode(file_get_contents('php://input'), true) ?? [];
        $contactoId = (int)($body['contacto_id'] ?? 0);

        if ($contactoId <= 0) {
            http_response_code(400);
            return ['error' => 'contacto_id requerido'];
        }

        // Buscar contacto
        $stmt = $pdo->prepare('SELECT id, instancia_id, numero, nombre, tipo FROM contactos WHERE id = ?');
        ...
        // ¿Ya existe un lead para este contacto + instancia?
        $stmt = $pdo->prepare('SELECT id FROM crm_leads WHERE contacto_id = ? AND instancia_id = ? LIMIT 1');
        $stmt->execute([$contactoId, (int)$contacto['instancia_id']]);
        $existente = $stmt->fetch();
        if ($existente) {
            return ['ok' => true, 'lead_id' => (int)$existente['id'], 'already_exists' => true];
        }

        // Crear el lead (sin asignar a nadie — todos los vendedores de la instancia lo ven)
        $stmt = $pdo->prepare("
            INSERT INTO crm_leads
                (instancia_id, contacto_id, numero, nombre, estado, usuario_asignado_id, created_at, updated_at)
            VALUES (?, ?, ?, ?, 'nuevo', NULL, NOW(), NOW())
        ");
        $stmt->execute([
            (int)$contacto['instancia_id'],
            $contactoId,
            $contacto['numero'],
        ...
        return ['ok' => true, 'lead_id' => $leadId, 'already_exists' => false];
    }

    /**
     * GET /api/crm/my-limit?instancia_id=X
     * Devuelve el límite del vendedor en esa instancia y cuántos chats activos tiene.
     * NO cuenta grupos (los grupos no consumen cupo).
     */
    public function myLimit(): array
    {
        $pdo  = Database::connect();
        $userId      = (int)($_REQUEST['_user_id'] ?? 0);
        $instanciaId = (int)($_GET['instancia_id'] ?? 0);

        if ($userId <= 0 || $instanciaId <= 0) {
            return ['max_chats_activos' => 0, 'chats_activos_actuales' => 0];
        }
        ...
        $stmt = $pdo->prepare("
            SELECT
              ...
              ) AS chats_activos_actuales
            FROM usuarios_instancias ui
            WHERE ui.usuario_id   = ?
              AND ui.instancia_id = ?
            LIMIT 1
        ");
        $stmt->execute([$userId, $instanciaId, $userId, $instanciaId]);
        $row = $stmt->fetch();
        ...
        return [
            'max_chats_activos'      => (int)$row['max_chats_activos'],
            'chats_activos_actuales' => (int)$row['chats_activos_actuales'],
        ];
    }

    /**
     * Envía media directamente a Evolution API usando /message/sendMedia
     * Para videos y otros tipos no cubiertos por los métodos existentes
     */
    private function enviarMediaEvolution($evolutionService, $numero, $base64, $mediatype, $mimetype, $filename, $caption = '')
    {
        // Usar reflexión para llamar al método request protegido de EvolutionService
        $reflection = new \ReflectionClass($evolutionService);
        $requestMethod = $reflection->getMethod('request');
        $requestMethod->setAccessible(true);
        
        $endpoint = "/message/sendMedia/" . $this->getInstanceName($evolutionService);
        $body = [
            'number'    => $this->formatearNumeroEvolution($evolutionService, $numero),
            'mediatype' => $mediatype,
            'media'     => $base64,
            'mimetype'  => $mimetype,
            'fileName'  => $filename,
        ...
        return $requestMethod->invoke($evolutionService, 'POST', $endpoint, $body);
    }
    
    private function getInstanceName($evolutionService)
    {
        $reflection = new \ReflectionClass($evolutionService);
        $instanceProp = $reflection->getProperty('instance');
        $instanceProp->setAccessible(true);
        return $instanceProp->getValue($evolutionService);
    }
    
    private function formatearNumeroEvolution($evolutionService, $numero)
    {
        $reflection = new \ReflectionClass($evolutionService);
        $formatMethod = $reflection->getMethod('formatearNumero');
        return $formatMethod->invoke($evolutionService, $numero);
    }
}
