...
class InstancesController
{
    ...
    public function getUsers(string $id): array
    {
        ...
        // Normalizar tipos
        foreach ($rows as &$r) {
            $r['max_chats_activos']      = (int)($r['max_chats_activos'] ?? 0);
            $r['chats_activos_actuales'] = (int)($r['chats_activos_actuales'] ?? 0);
        }
        return ['users' => $rows];
    }
    ...
    public function setUsers(string $id): array
    {
        ...
        // - Nuevo: { users: [{id, max_chats_activos}] }
        // - Viejo: { user_ids: [1,2,3] }  → max queda en 0 (sin límite)
        $items = [];
        if (!empty($input['users']) && is_array($input['users'])) {
            foreach ($input['users'] as $u) {
                ...
                $items[] = [
                    'id'  => $uid,
                    'max' => max(0, (int)($u['max_chats_activos'] ?? 0)),
                ];
            }
        } elseif (!empty($input['user_ids']) && is_array($input['user_ids'])) {
            foreach ($input['user_ids'] as $uid) {
                $items[] = ['id' => (int)$uid, 'max' => 0];
            }
        }

        $pdo = Database::connect();

        // Borrar asignaciones que ya no estén
        if (empty($items)) {
            $pdo->prepare('DELETE FROM usuarios_instancias WHERE instancia_id = ?')->execute([(int)$id]);
        } else {
            $ids = array_column($items, 'id');
            $placeholders = implode(',', array_fill(0, count($ids), '?'));
            $del = $pdo->prepare("
                DELETE FROM usuarios_instancias
                WHERE instancia_id = ?
                  AND usuario_id NOT IN ($placeholders)
            ");
            $del->execute(array_merge([(int)$id], $ids));
        }

        // Insertar/actualizar (depende de UNIQUE (usuario_id, instancia_id))
        $stmt = $pdo->prepare("
            INSERT INTO usuarios_instancias (usuario_id, instancia_id, max_chats_activos)
            VALUES (?, ?, ?)
            ON DUPLICATE KEY UPDATE max_chats_activos = VALUES(max_chats_activos)
        ");
        foreach ($items as $it) {
