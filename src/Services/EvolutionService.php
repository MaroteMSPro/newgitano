...
class EvolutionService
{
    ...
    public function enviarTexto(string $numero, string $mensaje, ?string $quotedMsgId = null): array
    {
        return $this->request('POST', "/message/sendText/{$this->instance}", [
            'number' => $this->formatearNumero($numero),
            'text'   => $mensaje,
            'quoted' => $quotedMsgId ? ['key' => ['remoteJid' => $this->formatearNumero($numero) . '@s.whatsapp.net', 'fromMe' => false, 'id' => $quotedMsgId]] : null,
        ]);
    }
     ...
     */
    /**
     * Escanea TODAS las páginas del historial para encontrar grupos únicos.
     * Corta cuando lleva N páginas sin encontrar grupos nuevos (early exit).
     */
    /**
     * Escanea el historial de mensajes saltando páginas para encontrar grupos únicos.
     * Lee páginas salteadas (cada N páginas) para cubrir todo el historial rápidamente.
     */
    /**
     * Escanea un chunk del historial buscando grupos únicos.
     * @param int $pageFrom  Página de inicio
     * @param int $chunkSize Cuántas páginas leer en este chunk
     * @return array ['grupos' => [...], 'totalPages' => N, 'nextPage' => N|null]
     */
    public function escanearChunkGrupos(int $pageFrom = 1, int $chunkSize = 15): array
    {
        $grupos     = [];
        $totalPages = null;

        for ($page = $pageFrom; $page < $pageFrom + $chunkSize; $page++) {
            if ($totalPages !== null && $page > $totalPages) break;

            $res     = $this->buscarMensajes([], 50, $page);
        ...
        $nextPage = ($pageFrom + $chunkSize <= ($totalPages ?? 1))
            ? $pageFrom + $chunkSize
            : null;
    ...
    {
        $res = $this->request('GET', "/group/participants/{$this->instance}?groupJid=" . urlencode($groupJid));
        // wrapper {success, http_code, data}
        return $res['data'] ?? $res;
    }

    /**
     * Devuelve count de participantes + lista de números de un grupo.
     */
    public function obtenerInfoGrupo(string $groupJid): array
    {
        $res   = $this->obtenerParticipantesGrupo($groupJid);
        $parts = $res['participants'] ?? [];
        $lista = [];
        foreach ($parts as $p) {
            $phone = $p['phoneNumber'] ?? $p['id'] ?? '';
            $phone = str_replace(['@s.whatsapp.net', '@c.us', '@lid'], '', $phone);
            $lista[] = ['numero' => $phone, 'admin' => $p['admin'] ?? null];
        }
        return ['size' => count($lista), 'participantes' => $lista];
    }
